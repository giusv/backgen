                           ...
                           (mapcar (lambda (plst)
                                   ...
                                   (mapcar #'cadr 
                                           (remove-if-not (lambda (elem) 
                                                          ...
                                                          db))))
                         tables))))
(indicatore sco-1 (soggetto) (durata occorrenze)
  (>= (cluster (sinistri soggetto 			     
                         (or (coinvolto)
                             (interessato)
                             (richiedente)
                             (responsabile)))
...
(multiple-value-bind (*sinistri-table* *soggetti-table*) 
    ;; (flatten-db (sco0 :id 101 :days 180 :occurrences 3))
    (pprint (flatten-db (sco0 :id 101 :days 180 :occurrences 3)))
  ;; (equijoin *sinistri-table* *soggetti-table* :id-sini)
  ;; (sinistri 103)
  ;; (restrict (sinistri 101)
  ;;           #'(lambda (row)
  ;;               (getf row :d-flg-respons)))
  ;; (pprint (esiste (cluster (restrict (sinistri 101)
  ;;                                    #'(lambda (row)
  ;;                                        (getf row :d-flg-respons)))
  ;;                          (* 180 86400))
  ;;                 3))
  ...
  (pprint (sco-1 101 (* 180 86400) 3))
  ;; (test-sco1 101 (* 180 86400))
  )
;; (pprint *database*)
