...
(defun sco4 ()
  (let ((days 180)
    ...
    (tl-and (tl-exists (sini gv-ind-stat-sini) 
                (:id-sini id-true :d-data-accad (tl-timestamp date))
              (tl-exists (sogg gv-ind-stat-sogg-sini) 
                  (:id-sini id-true :id-sogg id-true)))
            (tl-forall i occurrences
              (let ((data-accad (- date (* interval 86400))))) (tl-exists (sini gv-ind-stat-sini) 
                     (:id-sini (uniq id-true i) :d-data-accad (tl-random-timestamp  ,date))
                   (tl-exists (sogg gv-ind-stat-sogg-sini) 
                       (:id-sini (tl-retrieve :id-sini sini) :id-sogg ,id-sogg
                                 ,@sogg-flags)))))))
