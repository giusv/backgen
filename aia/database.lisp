...
(indicatore test-sco3 (soggetto) (durata occorrenze lesi)
  (cond ((ind-ge (cluster (sinistri-soggetto soggetto (and (ind-ge (numero-lesi t) lesi)
                                              (coinvolto-diretto t)))
                      durata)
             occorrenze) (values t t))
        ((not (ind-ge (cluster (sinistri-soggetto soggetto (and (ind-ge (numero-lesi nil) lesi)
...
(let ((res (apply #'append (loop for i from 101 to 102 collect
                                (let ((db (random-sogg :id i :days 180 :occurrences (1+ (random 2)) :lesi 5 :delay 20)))
  ...
  (let ((filename "D:/Dati/Profili/m026980/workspace/app/src/main/resources/data.sql")) 
    (pprint filename)
    (write-file filename (tl-ddl res)))
  (pprint res))
