...
(defun cluster (series duration sini &optional (accessor :d-data-accad))
		   ...
		   (cluster-helper (cdr series)
				   ...
				   (cons (list head)
					 (mapcar #'(lambda (cluster) 
							 ...
							 cluster))
						 acc)))))))
                                        ;(pprint (cluster-helper series duration nil))
    (aif (remove-if-not (lambda (clust)
                          (some (lambda (row)
                                  (equal sini (getf row :id-sini)))
                                clust))
                        (cluster-helper series duration nil))
         (apply #'max (mapcar (lambda (elem) (- (length elem) 1)) it))
         0)))
...
(defmacro sinistri-soggetto (sini sogg &optional (condition t))
  `(stable-sort (restrict (equijoin *soggetti-table* *sinistri-table* :id-sini)
                          #'(lambda (row) 
                              (or (and (ind-eq (getf row :id-sogg) ,sogg)
                                       (ind-eq (getf row :id-sini) ,sini)) 
                                  (and (ind-eq (getf row :id-sogg) ,sogg)
                                       ,condition))))
                #'<= 
                :key (lambda (row) 
                       (getf row :d-data-accad))))
...
(defmacro sinistri-veicolo (sini veic &optional (condition t))
  `(stable-sort (restrict (equijoin *veicoli-table* *sinistri-table* :id-sini)
                          #'(lambda (row) 
                              (or (and (ind-eq (getf row :id-targa) ,veic)
                                       (ind-eq (getf row :id-sini) ,sini)) 
                                  (and (ind-eq (getf row :id-targa) ,veic)
                                       ,condition))))
                #'<= 
