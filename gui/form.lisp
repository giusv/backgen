...
(defprim form% (name schema element)
  ...
  (:req (path namelist) (paragraph 
                (normal "Form identificato con ~a collegato al seguente formato dati:" (lower-camel name)) 
                (synth :req schema)
                (synth :req element path (append* namelist name))
                (normal "Esso produce il seguente oggetto JSON:")
                (verbatim (doc (synth :string (synth :model this))))))
...
(defprim bnd (name element)
  ...
  (:req (path namelist) (synth :req element path (append* namelist name)))
  (:brief (path namelist) (synth :req this path namelist))
...
(defprim obj% (name schema bindings element)
  (:pretty () (list 'obj (list :name name :schema (synth :pretty schema) 
                               ...
                               :element (synth :pretty element))))
  (:req (path namelist) (paragraph 
                         (normal "Sezione identificata con ~a collegata al seguente formato dati:" (lower-camel name)) 
                         (synth :brief schema)
                         (synth :req element path (append* namelist name))
                         (normal "Essa produce il seguente oggetto JSON:")
                         (verbatim (doc (synth :string (synth :model this))))))
  (:brief (path namelist) (synth :req this path namelist))
  (:model () (apply #'jobject (apply #'append (synth-all :model bindings))))
  (:reqlist (path namelist) nil)
  (:form-template (loopvar indexes) (seq (synth-all :form-template bindings loopvar indexes)))
...
(defprim arr% (name schema element)
  (:pretty () (list 'arr (list :name name 
                               ...
                               :element (synth :pretty element))))
  (:req (path namelist) (paragraph 
                (normal "Sezione dinamica identificata con ~a collegata al seguente formato dati:" (lower-camel name)) 
                (synth :brief schema) 
                (synth :req element path (append* namelist name))
                (normal "Essa produce il seguente oggetto JSON:")
                (verbatim (doc (synth :string (synth :model this))))))
