  ...
  (:doc () (labels ((stringify (item)
             ...
             (let ((attributes (rest-key args)) 
               ...
               (if (null body)
                   ...
                   (multiple-value-bind (cnc nst)
                       (if (and (= 1 (length body)) (synth :text (car body)))
                           (values #'doc:hcat #'identity)
                           (values #'doc:vcat (lambda (x) (doc:nest 4 x)))) 
                     (funcall cnc (open-tag attributes)
                              (funcall nst (apply cnc (synth-all :doc body)))
                              (close-tag)))))))
  (:text () nil))
