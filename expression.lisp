...
(defprim const (lit)
  ...
  (:url ()  (cond ((stringp lit) (doc:text "~a" lit))
                  ((numberp lit) (doc:text "~a" lit))
                  ((symbolp lit) (doc:text "~a" (string-downcase lit)))
  ...
  (:template () (cond ((stringp lit) (doc:text "~a" lit))
                      ...
                      ((eq t lit) (doc:text "true"))
                      (t (error "unknown constant type"))))
  (:java-implementation (cont &rest args) (apply cont (java:java-const lit) args))
...
(defprim cat (&rest exps)
  ...
  (:java-implementation (cont &rest args) (apply cont (reduce #'java-+ exps) args)))
...
(defmacro defbexp (operator &optional representation (arity 0))
  (let ((name (symb "+" operator "+")))
    `(defprim ,name 
       ...
       (:java-implementation (cont &rest args) (apply cont ,(if (eq arity 'unbounded)
                                                  `(,(intern (mkstr "JAVA-" operator) "JAVA") (synth-all :java-implementation exps #'identity))
                                                  `(,(intern (mkstr "JAVA-" operator) "JAVA") ,@(loop for i from 1 to arity collect `(synth :java-implementation ,(symb "EXP" i) #'identity)))) args))
