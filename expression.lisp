...
(defprim attr (name exp)
  ...
  (:blub () (java:java-chain (java:java-dynamic (if (functionp name)
                                                (synth :name name)
                                                name)) 
                           (java:java-call (symb "GET-" exp))))
...
(defprim cat (&rest exps)
  ...
  (:blub () (reduce #'java-+ exps)))

(defprim variab (name)
  (:pretty () (list 'variab (list :name name)))
  (:string () (textify name))
  (:blub () (java:java-dynamic name)))
  ...
  (let ((name (symb "+" operator "+")))
    `(defprim ,name 
       ...
       (:blub () ,(if (eq arity 'unbounded)
                      `(,(intern (mkstr "JAVA-" operator) "JAVA") (synth-all :blub exps))
                      `(,(intern (mkstr "JAVA-" operator) "JAVA") ,@(loop for i from 1 to arity collect `(synth :blub ,(symb "EXP" i))))))
