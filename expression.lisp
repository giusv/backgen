...
(defprim const (lit)
  ...
  (:implementation (cont &rest args) (apply cont (java:java-const lit) args))
...
(defprim attr (name exp)
  ...
  (:implementation (cont &rest args) (apply cont (java:java-chain (java:java-dynamic (if (functionp name)
                                                                     (synth :name name)
                                                                     name)) 
...
(defprim cat (&rest exps)
  ...
  (:implementation (cont &rest args) (apply cont (reduce #'java-+ exps) args)))
...
(defmacro defbexp (operator &optional representation (arity 0))
  (let ((name (symb "+" operator "+")))
    `(defprim ,name 
       ...
       (:implementation (cont &rest args) (apply cont ,(if (eq arity 'unbounded)
                                                  `(,(intern (mkstr "JAVA-" operator) "JAVA") (synth-all :implementation exps #'identity))
                                                  `(,(intern (mkstr "JAVA-" operator) "JAVA") ,@(loop for i from 1 to arity collect `(synth :implementation ,(symb "EXP" i) #'identity)))) args))
